...
public class PhysicsAnimationHandler implements AnimationHandler {
    ...
    public void renderTransform(FallingTreeEntity entity, float entityYaw, float partialTicks, PoseStack matrixStack) {
        final float yaw = Mth.wrapDegrees(io.github.steveplays28.dynamictreesfabric.util.MathHelper.angleDegreesInterpolate(entity.yRotO, entity.getYRot(), partialTicks));
        final float pit = Mth.wrapDegrees(io.github.steveplays28.dynamictreesfabric.util.MathHelper.angleDegreesInterpolate(entity.xRotO, entity.getXRot(), partialTicks));
